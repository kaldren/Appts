...
public interface IAppointmentsDb
{
    ...
    Task<Appointment?> GetAppointmentByClientIdAsync(Guid clientId, CancellationToken cancellationToken);
    Task<List<Appointment>> GetAllClientAppointmentsAsync(Guid clientId, CancellationToken cancellationToken);
}
